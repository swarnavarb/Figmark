 ...
 */
/**
 * Where a lot travels, in the seller's own words: "Guangzhou, CN → Mumbai, IN".
 *
 ...
 * said where it is going yet.
 */
export function laneOf(lot: Pick<Lot, 'origin' | 'destination'>): string {
  return `${lot.origin?.trim() || 'Origin'} → ${lot.destination?.trim() || 'Destination'}`;
}
